...
const ProjectCardResumed: React.FC<ProjectCardProps> = ({
  ...
  return (
    <Card className="w-full max-w-lg border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm hover:shadow-md transition-transform duration-300 transform hover:scale-105 focus-within:scale-105 active:scale-95">
      {/* Card Header */}
      <CardHeader className="p-4">
        <CardTitle className="text-lg font-semibold text-slate-900 dark:text-white">
          {title}
        </CardTitle>
        <p className="text-sm flex flex-row items-center space-x-4 text-slate-600 dark:text-slate-400">
      ...
      <CardContent className="px-4 py-2 space-y-3">
        <div className="w-full h-48 overflow-hidden rounded-lg transition-transform duration-300 transform hover:scale-105">
          <Image
            src={thumbnail}
            alt={title}
